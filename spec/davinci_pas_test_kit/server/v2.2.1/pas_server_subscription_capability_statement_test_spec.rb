RSpec.describe DaVinciPASTestKit::DaVinciPASV221::PASServerSubscriptionCapabilityStatementTest, :runnable do
  ...
  def capability_statement(interactions: %w[create update delete], subscription_resource: true, server_rest: true)
    rest = []
    resources = []
  ...
  def stub_capability_statement(body)
    stub_request(:get, "#{server_endpoint}/metadata")
      .to_return(status: 200, body:, headers: { 'Content-Type' => 'application/fhir+json' })
  end
  ...
  it 'passes when the server declares all required Subscription interactions' do
    ...
    expect(run(test, server_endpoint:).result).to eq('pass')
  end

  it 'fails when the server does not declare a Subscription resource' do
    stub_capability_statement(capability_statement(subscription_resource: false))

    expect(run(test, server_endpoint:).result).to eq('fail')
  end

  it 'fails when the Subscription resource is missing a required interaction' do
    stub_capability_statement(capability_statement(interactions: %w[create update]))

    expect(run(test, server_endpoint:).result).to eq('fail')
  end

  it 'fails when the CapabilityStatement has no server-mode rest entry' do
    stub_capability_statement(capability_statement(server_rest: false))
